...
bb.addEventListener("click", h2Write);
function h2Write(){
    const h2 = document.querySelectorAll('h2');
    h2.forEach(e =>{
        e.innerText = rand(1, 6);
    });
}
...
const bb2=document.querySelector('h3 + button');
const arrayNumbers = [];
let sum=0;
bb2.addEventListener("click", addToArray);
...
const firstUl=document.querySelector('h3 + button + ul');
africa.forEach(e => {
    //I think empty should not be readed so skiped empty ones
    if(e.length !== 0){
...
let str='';
const secondUl=document.querySelector('button + button + ul');
australia.forEach(e => {
    (e==='Dingo') ? str+=`<li style="background-color: blue;">${e}</li>` : str+=`<li>${e}</li>`;
});
secondUl.innerHTML = str;
